	...
	class Mega_Slider
	{
		...
		public static function init()
		{
			register_post_type(
				'mega-slider',
				apply_filters(
					'mega-slider_register_post_type',
					array(
						'labels' => array(
...
?>
		...
		public static function slider($id)
		{
			...
			if ($id) {
				...
				$slides = wbp_ssv_posts_by_term($ssv_cat_ids, $cat_ids, $args);
						...
						<?php foreach ($slides as $i => $slide) { ?>
							<div class="mega-slider__row<?php if (0 == $i) { ?> mega-slider__row--active<?php } ?>">
								...
								<span class="mega-slider__row__label">
									...
									<a class="mega-slider__row__link" href="<?php echo get_post_permalink($slide->ID, false, true); ?>"><?php _e('Read more...'); ?></a>
								</span>
							</div>
						<?php } ?>
					</div>
